...
/* loaded from: cloudhosting-0.0.1.jar:BOOT-INF/lib/aspectjweaver-1.9.19.jar:aj/org/objectweb/asm/Constants.class */
final class Constants {
    ...
    static boolean isWhitelisted(String internalName) {
        if (internalName.startsWith("aj/org/objectweb/asm/")) {
            return internalName.contains("Test$") || Pattern.matches(new StringBuilder().append("aj/org/objectweb/asm/util/Trace").append("(Annotation|Class|Field|Method|Module|RecordComponent|Signature)").append("Visitor(\\$.*)?").toString(), internalName) || Pattern.matches(new StringBuilder().append("aj/org/objectweb/asm/util/Check").append("(Annotation|Class|Field|Method|Module|RecordComponent|Signature)").append("Adapter(\\$.*)?").toString(), internalName);
        }
        return false;
    }
