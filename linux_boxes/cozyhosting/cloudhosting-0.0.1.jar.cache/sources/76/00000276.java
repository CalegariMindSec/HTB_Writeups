...
/* loaded from: cloudhosting-0.0.1.jar:BOOT-INF/lib/logback-core-1.4.5.jar:ch/qos/logback/core/util/Duration.class */
public class Duration {
    ...
    public static Duration valueOf(String durationStr) {
        ...
        if (matcher.matches()) {
            ...
            if (unitStr.equalsIgnoreCase(EscapedFunctions.SECOND) || unitStr.equalsIgnoreCase("seconde")) {
                return buildBySeconds(doubleValue);
            }
            if (unitStr.equalsIgnoreCase(EscapedFunctions.MINUTE)) {
                return buildByMinutes(doubleValue);
            }
            if (unitStr.equalsIgnoreCase(EscapedFunctions.HOUR)) {
