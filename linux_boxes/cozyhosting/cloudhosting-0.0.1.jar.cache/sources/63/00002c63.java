package org.hibernate.boot.jaxb.hbm.spi;

import jakarta.xml.bind.annotation.XmlEnum;
import jakarta.xml.bind.annotation.XmlType;

@XmlType(name = "NotFoundEnum", namespace = "http://www.hibernate.org/xsd/orm/hbm")
@XmlEnum
/* loaded from: cloudhosting-0.0.1.jar:BOOT-INF/lib/hibernate-core-6.1.6.Final.jar:org/hibernate/boot/jaxb/hbm/spi/JaxbHbmNotFoundEnum.class */
public enum JaxbHbmNotFoundEnum {
    EXCEPTION("exception"),
    IGNORE("ignore");
    
    private final String value;

    JaxbHbmNotFoundEnum(String v) {
        this.value = v;
    }

    public String value() {
        return this.value;
    }

    public static JaxbHbmNotFoundEnum fromValue(String v) {
        JaxbHbmNotFoundEnum[] values;
        for (JaxbHbmNotFoundEnum c : values()) {
            if (c.value.equals(v)) {
                return c;
            }
        }
        throw new IllegalArgumentException(v);
    }
}