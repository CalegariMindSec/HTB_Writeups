...
/* loaded from: cloudhosting-0.0.1.jar:BOOT-INF/lib/txw2-4.0.1.jar:com/sun/xml/txw2/output/IndentingXMLFilter.class */
public class IndentingXMLFilter extends XMLFilterImpl implements LexicalHandler {
    ...
    public void comment(char[] ch2, int start, int length) throws SAXException {
        ...
        if (this.lexical != null) {
            this.lexical.comment(ch2, start, length);
        }
    }
    ...
    private void doIndent() throws SAXException {
        if (this.depth > 0) {
            char[] ch2 = this.indentStep.toCharArray();
            for (int i = 0; i < this.depth; i++) {
                characters(ch2, 0, ch2.length);
            }
        }
    }
}